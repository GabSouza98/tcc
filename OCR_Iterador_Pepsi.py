...
for IMAGE_PATH in glob.glob('./V154/Pepsi/*.png'):
    ...
    cv2.imwrite(f'./V154/Pepsi/TRATADO/denoised_{amostra}.png', img)   #PADRÃO PARA TESTAR NO GOOGLE API    
...
lista = []
for IMAGE_PATH in glob.glob('./V154/Pepsi/TRATADO/*denoised*.png'):
    ...
    cv2.imwrite(f'./V154/Pepsi/TRATADO/deskewed_{amostra}.png', img_deskewed)
    cv2.imwrite(f'./V154/Pepsi/TRATADO/dilated_{amostra}.png', img_dilated)   
    
    with open('./V154/Pepsi/TRATADO/EASYOCR/resultados_easyocr.txt', 'a+') as f:
        f.write(f'Resultado denoised {amostra}\n')
        for line in lista:
            f.write(line)
            f.write('\n')
        f.write('###########\n\n')
...
# distorcer = ['resized','cropped','binario']
# print("COMEÇANDO O TRATAMENTO ARC DISTORTION")
# for tratamento in distorcer:
#     for IMAGE_PATH in glob.glob(f'./V154/Pepsi/TRATADO/*{tratamento}*.png'):
...
#             cv2.imwrite(f'./V154/Pepsi/TRATADO/DISTORTED/{tratamento}_{amostra}.png', retval)  


print("INICIANDO A LEITURA COM GOOGLE VISION")
for IMAGE_PATH in glob.glob('./V154/Pepsi/TRATADO/*deskewed*.png'):
    
    amostra = retorna_nome_amostra(IMAGE_PATH)
    print("LENDO COM GOOGLE VISION A IMAGEM ", amostra)
    texto = detect_text(IMAGE_PATH)
    with open('./V154/Pepsi/TRATADO/GOOGLEVISION/resultados_googlevision.txt', 'a+', encoding="utf-8") as f:
        f.write(f'Resultado Final deskewed GoogleVision {amostra}\n')
        f.write(texto)
        f.write("\n")
        f.write(verifica_conformidade_pepsi(texto))                
        f.write('\n###########\n\n')    
