...
img = cv2.imread('FOTOS_GRF/1.jpg') #em BGR

im_gray = Image.open('FOTOS_GRF/1.jpg').convert('LA')

im_rgb = Image.open('FOTOS_GRF/1.jpg')
...
threshold = 190 
black_white = resized_im.point(lambda p: p > threshold and 255)
black_white.save('teste1.png')
#black_white.show()

gray = cv2.imread('teste1.png', cv2.IMREAD_GRAYSCALE)  #opcional, pois a imagem já esta em grayscale 
...
for i in range(a):
    ...
    cv2.imwrite('teste1_linhas.jpg',backtorgb)
...
# redBand      = multiBands[0].show()
# greenBand    = multiBands[1].show()
# blueBand     = multiBands[2].show()

# im_filtrada = im.filter(EDGE_ENHANCE_MORE)
# im_filtrada.save('FOTOS_GFR/Imfiltrada.jpg')
# im_filtrada.show()
